    ...
    for conn, conn_v in connections.items():
        ...
        if conn in ['ex_ex', 'ex_in', 'in_ex', 'in_in']:
            ...
            if conn.split("_")[0] == conn.split("_")[1] == "ex":
                #connect_pops(pop_ex, pop_ex, conn_v["conn_spec"], conn_v["syn_spec"])
                pass
            elif conn.split("_")[0] == conn.split("_")[1] == "in":
                #connect_pops(pop_in, pop_in, conn_v["conn_spec"], conn_v["syn_spec"])
                pass
            elif conn.split("_")[0] == "ex" and conn.split("_")[1] == "in":
                #connect_pops(pop_ex, pop_in, conn_v["conn_spec"], conn_v["syn_spec"])
                pass
            elif conn.split("_")[0] == "in" and conn.split("_")[1] == "ex":
                #connect_pops(pop_in, pop_ex, conn_v["conn_spec"], conn_v["syn_spec"])
                pass
